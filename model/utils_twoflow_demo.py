...
class TrainSetLoader(Dataset):
    ...
    def _sync_transform(self, img, imgreverse, mask):
        # random mirror
        # dropout1 = random.random()
        # dropout2 = random.random()
        # if dropout1 < dropout2:
...
class DemoLoader (Dataset):
    ...
    def __init__(self, dataset_dir, transform=None,base_size=512,crop_size=480,suffix='.png'):
        ...
        self.transform = transform
        self.images    = dataset_dir[0]
        self.images_reverse = dataset_dir[1]
        self.base_size = base_size
        self.crop_size = crop_size
        self.suffix    = suffix
    ...
    def img_preprocess(self):
        ...
        img_path_reverse = self.images_reverse
        img_reverse = Image.open(img_path_reverse).convert('RGB')

        # synchronized transform
        img  = self._demo_sync_transform(img)
        img_reverse = self._demo_sync_transform(img_reverse)
...
def save_Pred_GT(pred, labels, target_image_path, val_img_ids, num, suffix):
    ...
    predsss = np.array((pred > 0).cpu()).astype('int64') * 255  #大于０的元素设置为２５５，别的试着维０
